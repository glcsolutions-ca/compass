...
import { buildDefaultAzureDynamicSessionsHost } from "./hosts/azure-dynamic-sessions.js";
import { LocalProcessSessionHost } from "./hosts/local-process.js";
...
function toWsUrl(baseUrl: string): string {
  ...
  url.pathname = "/internal/session-agent/connect";
  url.search = "";
  url.hash = "";
  return url.toString();
}
...
export class SessionControlPlane {
  ...
  readonly #hosts = new Map<AgentExecutionHost, SessionHost>();
  ...
  private requireHost(executionHost: AgentExecutionHost): SessionHost {
...
export function buildDefaultSessionControlPlane(input: {
  ...
  const localWorkRoot = String(
    env.AGENT_SESSION_LOCAL_WORK_ROOT || ".artifacts/session-agents"
  ).trim();
