...
interface ThreadWebSocketServer {
  handleUpgrade(
    request: IncomingMessage,
    socket: Duplex,
    head: Buffer,
    callback: (socket: ThreadWebSocketConnection) => void
  ): void;
}
...
function parseThreadStreamRequest(request: IncomingMessage): {
  ...
  const match = /^\/v1\/threads\/([^/]+)\/stream$/u.exec(url.pathname);
  if (!match) {
    return null;
  }
...
function parseRuntimeStreamRequest(request: IncomingMessage): boolean {
  const rawUrl = String(request.url || "");
  const url = new URL(rawUrl, "http://localhost");
  if (url.pathname !== "/v1/runtime/stream") {
    return false;
  }
...
function sendEvent(socket: { send(data: string): void }, event: ThreadEventRecord): void {
  socket.send(
    JSON.stringify({
      type: event.method,
      method: event.method,
...
export function attachThreadWebSocketGateway(input: {
  server: Server;
  authService: AuthService | null;
  threadService: ThreadService | null;
  now: () => Date;
}): void {
  const wss = new WebSocketServer({ noServer: true }) as unknown as ThreadWebSocketServer;
  ...
  input.server.on("upgrade", (request, socket, head) => {
    void (async () => {
      ...
      if (!input.authService || !input.threadService) {
        respondUpgradeError(socket, 503, "Thread websocket is not configured");
        return;
      }
      ...
      wss.handleUpgrade(request, socket, head, (ws) => {
        void (async () => {
          try {
            ...
            const unsubscribe = input.threadService!.subscribeRuntimeNotifications((event) => {
              if (replaying) {
            ...
            const historical = await input.threadService!.listRuntimeNotifications({
              userId,
            ...
            try {
              ws.close(1011, "Thread stream setup failed");
            } catch {
              // ignore close errors
            }
          }
...
export const __internalThreadWebSocket = {
