...
import { parseAuthError } from "../auth/auth-service.js";
import type { SessionControlPlane } from "./session-control-plane.js";

interface SessionThreadWebSocketServer {
  handleUpgrade(
...
function matchesSessionAgentPath(request: IncomingMessage): boolean {
  ...
  return url.pathname === "/internal/runtime-agent/connect";
}
...
export function attachSessionAgentGateway(input: {
  ...
  const wss = new WebSocketServer({ noServer: true }) as unknown as SessionThreadWebSocketServer;
