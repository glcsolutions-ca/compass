...
import { __internalThreadServiceRuntime } from "./thread-service.js";

const { PostgresThreadService } = __internalThreadServiceRuntime;
...
describe("PostgresThreadService", () => {
  ...
  it("fails when parent turn reference is missing", async () => {
    const client = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rowCount: 0, rows: [] })
    ...
    await expect(
      ...
      status: 404,
      code: "AGENT_PARENT_TURN_NOT_FOUND"
    });
    expect(client.query).toHaveBeenCalledWith("rollback");
  });

  it("fails when source turn reference is missing", async () => {
    const client = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rowCount: 0, rows: [] })
    ...
    await expect(
      service.startTurn({
        ...
        threadId: "thread-1",
        text: "Hello",
        parentTurnId: "turn-existing",
        sourceTurnId: "missing-source",
        now: new Date("2026-03-03T00:00:00.000Z")
      })
    ).rejects.toMatchObject({
      status: 404,
      code: "AGENT_SOURCE_TURN_NOT_FOUND"
    });
    expect(client.query).toHaveBeenCalledWith("rollback");
  });

  it("completes startTurn with runtime output and emits completion events", async () => {
    const startClient = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rowCount: 0, rows: [] })
        .mockResolvedValueOnce({ rowCount: 0, rows: [] })
        .mockResolvedValueOnce({ rowCount: 1, rows: [createTurnRow({ status: "inProgress" })] })
    ...
    const completeClient = {
      query: vi
        ...
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [createEventRow({ id: 11, turn_id: "turn-start", method: "item.delta" })]
        })
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [createEventRow({ id: 12, turn_id: "turn-start", method: "runtime.metadata" })]
        })
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [createEventRow({ id: 13, turn_id: "turn-start", method: "turn.completed" })]
        })
        .mockResolvedValueOnce({ rowCount: 1, rows: [] })
        .mockResolvedValueOnce({ rowCount: 0, rows: [] }),
      release: vi.fn()
    ...
    const pool = {
      ...
      connect: vi.fn().mockResolvedValueOnce(startClient).mockResolvedValueOnce(completeClient),
      end: vi.fn(async () => {})
    };
    const service = new PostgresThreadService({
      pool: pool as never,
    ...
    expect(result.outputText).toBe("runtime-output");
    expect(runtimeDriver.bootstrapSession).toHaveBeenCalledTimes(1);
    expect(runtimeDriver.runTurn).toHaveBeenCalledTimes(1);
    expect(startClient.query).toHaveBeenCalledWith("commit");
    expect(completeClient.query).toHaveBeenCalledWith("commit");
  });

  it("marks turn as error when runtime execution fails", async () => {
    const startClient = {
      query: vi
        .fn()
        .mockResolvedValueOnce({ rowCount: 0, rows: [] })
        ...
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [createEventRow({ id: 20, turn_id: "turn-error", method: "turn.started" })]
        })
        .mockResolvedValueOnce({ rowCount: 0, rows: [] }),
      release: vi.fn()
    };
    const completeClient = {
      query: vi
        ...
        .mockResolvedValueOnce({
          rowCount: 1,
          rows: [createEventRow({ id: 21, turn_id: "turn-error", method: "turn.completed" })]
        })
        .mockResolvedValueOnce({ rowCount: 1, rows: [] })
        .mockResolvedValueOnce({ rowCount: 0, rows: [] }),
      release: vi.fn()
    ...
    expect(runtimeDriver.runTurn).toHaveBeenCalledTimes(1);
    expect(completeClient.query).not.toHaveBeenCalledWith(
