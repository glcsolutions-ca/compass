...
import { __internalThreadServiceMapping } from "../../src/modules/threads/thread-service.js";
...
describe("agent service row mapping", () => {
  it("maps thread rows that contain Date timestamp objects", () => {
    ...
    const mapped = __internalThreadServiceMapping.mapThreadRow({
      thread_id: "thread-1",
  ...
  it("maps turn rows that contain Date timestamp objects", () => {
    ...
    const mapped = __internalThreadServiceMapping.mapTurnRow({
      turn_id: "turn-1",
      thread_id: "thread-1",
      status: "completed",
      execution_mode: "cloud",
  ...
  it("maps event rows that contain Date timestamp objects", () => {
    ...
    const mapped = __internalThreadServiceMapping.mapEventRow({
      id: 7,
      thread_id: "thread-1",
      turn_id: "turn-1",
      method: "turn.started",
