...
vi.mock("~/features/chat/thread-client", () => ({
  listChatThreadEventsClient: vi.fn(),
  parseThreadStreamEventPayload: vi.fn()
}));
...
describe("agent transport", () => {
  ...
  it("streams websocket events, falls back to polling on disconnect, and dedupes cursors", async () => {
    ...
    vi.mocked(parseThreadStreamEventPayload).mockReturnValue(streamEvent);
    vi.mocked(listChatThreadEventsClient).mockResolvedValue({
    ...
    expect(vi.mocked(listChatThreadEventsClient)).toHaveBeenCalledWith({
      threadId: "thread-1",
      cursor: 5,
      limit: 200
    });
