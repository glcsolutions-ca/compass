...
function createWebSocketUrl(threadId: string, cursor: number): string {
  const url = new URL(`/v1/threads/${encodeURIComponent(threadId)}/stream`, window.location.origin);
  url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
  url.searchParams.set("cursor", String(cursor));
  return url.toString();
}

export function startThreadTransport(options: ThreadTransportOptions): ThreadTransportHandle {
  ...
  const connectWebSocket = () => {
    ...
    websocket.onmessage = (messageEvent) => {
      ...
      try {
        ...
        const event = parseThreadStreamEventPayload(parsed);
        if (event) {
          applyEvent(event);
        }
      } catch {
