...
export async function fetchRuntimeAccountRead(refreshToken = false): Promise<RuntimeAccountState> {
  const response = await fetch("/v1/runtime/account/read", {
    method: "POST",
    credentials: "include",
    headers: {
...
export async function postRuntimeLoginStart(
  ...
  const response = await fetch("/v1/runtime/account/login/start", {
    method: "POST",
    credentials: "include",
    headers: {
      "content-type": "application/json"
...
export async function postRuntimeLoginCancel(loginId: string): Promise<void> {
  const response = await fetch("/v1/runtime/account/login/cancel", {
    method: "POST",
    credentials: "include",
    headers: {
      "content-type": "application/json"
...
export async function postRuntimeLogout(): Promise<void> {
  const response = await fetch("/v1/runtime/account/logout", {
    method: "POST",
    credentials: "include"
  });
  const payload = await readResponseJson(response);
...
export async function postRuntimeRateLimitsRead(): Promise<RuntimeRateLimitsState> {
  const response = await fetch("/v1/runtime/account/rate-limits/read", {
    method: "POST",
    credentials: "include"
  });
  const payload = await readResponseJson(response);
...
export function subscribeRuntimeStream(onEvent: (event: RuntimeNotification) => void): () => void {
  ...
  const connect = () => {
    ...
    const endpoint = new URL("/v1/runtime/stream", window.location.origin);
