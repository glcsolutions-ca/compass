...
describe("migration-policy-lib", () => {
  it("passes policy validation for a valid migration set", async () => {
    await withTempDir(async (migrationsDir) => {
      ...
      await writeMigration(migrationsDir, "20260225170000000_baseline_platform_schema.mjs");
      ...
      expect(result.migrationFiles).toEqual(["20260225170000000_baseline_platform_schema.mjs"]);
    });
  });
  ...
  it("fails directory validation when non-.mjs migration files are present", async () => {
    await withTempDir(async (migrationsDir) => {
      await writeMigration(migrationsDir, "20260225170000000_baseline_platform_schema.mjs");
      await writeMigration(migrationsDir, "20260225170001000_bad.sql", "select 1;\n");

      const result = await validateMigrationDirectory({ migrationsDir });
      expect(result.failures).toContain(
        "Non-.mjs file found in db/migrations: 20260225170001000_bad.sql"
      );
    });
  });
  ...
  it("fails policy validation when checksums drift", async () => {
    await withTempDir(async (migrationsDir) => {
      const checksumsPath = path.join(migrationsDir, "checksums.json");
      const fileName = "20260225170000000_baseline_platform_schema.mjs";
  ...
  it("writes checksums manifest with sorted migration keys", async () => {
    await withTempDir(async (migrationsDir) => {
      const checksumsPath = path.join(migrationsDir, "checksums.json");
      await writeMigration(migrationsDir, "20260225170002000_second.mjs");
      await writeMigration(migrationsDir, "20260225170001000_first.mjs");
      ...
      expect(Object.keys(manifest.files)).toEqual([
        "20260225170001000_first.mjs",
        "20260225170002000_second.mjs"
      ]);
      expect(manifest.files["20260225170001000_first.mjs"]).toBe(
        checksums["20260225170001000_first.mjs"]
      );
      expect(manifest.files["20260225170002000_second.mjs"]).toBe(
        checksums["20260225170002000_second.mjs"]
      );
    });
  });
});
