...
2. Route contract is fixed to `/`, `/login`, `/automations`, `/skills`, `/chat`, `/w/:workspaceSlug/chat/:threadId?`, `/workspaces`.
...
8. Chat onboarding is personal-first (`/chat` redirect) while chat execution remains workspace-scoped via `/w/:workspaceSlug/chat`.
9. Backend auth contract auto-provisions a first-class personal workspace membership for every authenticated user; frontend chat context must resolve from workspace memberships only (no hardcoded `"personal"` slug fallback).
