...
| ---------- | ------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| 2026-02-26 | Stage A | A-001       | Replace dual-static hook model with tiered local gates: pre-commit `pnpm exec lint-staged && pnpm test:quick`, pre-push `pnpm test:full`. | accepted   | Simplifies defaults while preserving strict fail-closed enforcement at push boundary and reducing manual formatting steps. | Stage B should assume `test:quick` is the canonical local fast gate and `test:full` is the deep gate.             |
...
| 2026-02-26 | Stage A | A-005       | Keep `test:quick` fail-closed and add `FMT001` format-check reason code with explicit remediation commands.                               | accepted   | Preserves strict evidence gating while improving operator and agent troubleshooting speed.                                 | Stage B should assume formatting gate failures are explicit and actionable via `FMT001` guidance.                 |
