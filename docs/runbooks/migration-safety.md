...
- Production mutation is serialized by `concurrency: production-mutation` in `.github/workflows/mainline-pipeline.yml`.
...
- First response: replay a previously accepted candidate SHA via `mainline-pipeline.yml`.
