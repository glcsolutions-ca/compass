...
- one shared session-agent protocol is used in every environment
- one shared session-agent runtime is used in every environment
- local development uses `local API -> local session agent`
- cloud execution uses `cloud API -> Azure Dynamic Sessions -> session agent`
- the session agent currently runs an echo responder instead of Codex
...
- `packages/session-agent-protocol/*`
- `packages/session-agent/*`
- `apps/api/src/agent-sessions/*`
...
- `POST /v1/agent/threads/:threadId/turns`
- `POST /v1/agent/threads/:threadId/runtime/launch`

The public session surface is `/v1/agent/...`. Local development defaults to
`executionMode=local` and `executionHost=desktop_local`.

## Runtime Flow

1. Client creates or reuses an agent thread through the API.
2. A turn hits the API control plane through `/v1/agent/threads/:threadId/turns`.
3. The API resolves the thread host:
   - `desktop_local` -> spawn or reuse a local session agent
   - `dynamic_sessions` -> bootstrap or reuse an Azure session agent
4. The session agent connects back to `/internal/session-agent/connect`.
5. The API sends `turn.run`.
6. The session agent replies with `turn.result`.
...
AGENT_SESSION_LOCAL_WORK_ROOT=.artifacts/session-agents
AUTH_MODE=mock
```
...
- `scripts/dev/session-prototype/create-session-pool.sh`
- `scripts/dev/session-prototype/start-api-tunnel.sh`
- `scripts/dev/session-prototype/smoke-chat-runtime.mjs`
...
2. Run the smoke test:
   ...
   ```bash
   node scripts/dev/session-prototype/smoke-chat-runtime.mjs
   ```
...
2. Expose a stable public API URL:
   ...
   ```bash
   PORT=3001 scripts/dev/session-prototype/start-api-tunnel.sh
   ```
...
- One session-agent protocol works for local and cloud execution.
- The same runtime process can run locally or in Azure.
- The `/v1/agent` surface can stay stable while the host changes underneath.
