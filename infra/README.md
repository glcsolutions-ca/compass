...
2. `cloud-deployment-pipeline.yml` runs acceptance jobs (`infra-readonly-acceptance`, `identity-readonly-acceptance`) against the frozen candidate.
3. `cloud-deployment-pipeline.yml` performs guarded production mutation (`deploy-approved-candidate`) when required, with `approve-control-plane` for infra/identity control-plane scopes.

## Replay and Determinism

Replay is part of the operating model:

- Replay via `cloud-deployment-pipeline.yml` `workflow_dispatch` with `candidate_sha=<sha>`.
