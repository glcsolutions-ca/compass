...
function toTurnWithOutputText(payload: TurnStartResponse): ChatTurn {
  return {
    ...payload.turn,
...
function toInterruptTurnWithOutputText(payload: TurnInterruptResponse): ChatTurn {
  return {
    ...payload.turn,
    outputText: null
  };
...
function toEventsResult(payload: { events: ChatEvent[] }): ChatEventsResult {
...
export async function createChatThread(
  ...
  const result = await client.POST("/v1/threads", {
    credentials: "include",
    body: {
...
export async function listChatThreads(
  ...
  const client = createRouteClient(request);
  const result = await client.GET("/v1/threads", {
    credentials: "include",
    params: {
      query: {
        workspaceSlug: payload.workspaceSlug,
...
export async function getChatThread(
  ...
  const client = createRouteClient(request);
  const result = await client.GET("/v1/threads/{threadId}", {
    credentials: "include",
    params: {
      path: {
        threadId
...
export async function listChatThreadsClient(payload: {
  workspaceSlug: string;
  state?: "regular" | "archived" | "all";
  limit?: number;
  baseUrl?: string;
  ...
  const parsed = ThreadListResponseSchema.safeParse(result.data);
  if (!parsed.success) {
    return [];
  }
...
export async function patchChatThreadClient(payload: {
  threadId: string;
  title?: string;
  archived?: boolean;
  baseUrl?: string;
  ...
  const client = createBrowserClient(resolveBrowserBaseUrl(payload.baseUrl));
  const result = await client.PATCH("/v1/threads/{threadId}", {
    credentials: "include",
    params: {
      path: {
  ...
  const parsed = ThreadPatchResponseSchema.safeParse(result.data);
  if (!parsed.success) {
...
export async function deleteChatThreadClient(payload: {
  threadId: string;
  baseUrl?: string;
}): Promise<{ deleted: true }> {
  const client = createBrowserClient(resolveBrowserBaseUrl(payload.baseUrl));
  const result = await client.DELETE("/v1/threads/{threadId}", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId
  ...
  const parsed = ThreadDeleteResponseSchema.safeParse(result.data);
  if (!parsed.success) {
...
export async function switchChatThreadMode(
  request: Request,
  payload: {
    threadId: string;
    executionMode: ChatExecutionMode;
  ...
  const client = createRouteClient(request);
  const result = await client.PATCH("/v1/threads/{threadId}/mode", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId
...
export async function startChatTurn(
  ...
  const client = createRouteClient(request);
  const result = await client.POST("/v1/threads/{threadId}/turns", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId
...
export async function interruptChatTurn(
  ...
  const client = createRouteClient(request);
  const result = await client.POST("/v1/threads/{threadId}/turns/{turnId}/interrupt", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId,
...
): Promise<ApiResult<ChatEventsResult>> {
  const client = createRouteClient(request);
  const result = await client.GET("/v1/threads/{threadId}/events", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId
...
export async function listChatThreadEventsClient(payload: {
  threadId: string;
  cursor?: number;
  limit?: number;
  baseUrl?: string;
  ...
  const parsed = ThreadEventsListResponseSchema.safeParse(result.data);
  if (!parsed.success) {
    return {
      events: [],
      nextCursor: 0
...
export async function appendChatThreadEventsBatchClient(payload: {
  threadId: string;
  events: Array<{
    turnId?: string;
    method: string;
  ...
  const client = createBrowserClient(resolveBrowserBaseUrl(payload.baseUrl));
  const result = await client.POST("/v1/threads/{threadId}/events:batch", {
    credentials: "include",
    params: {
      path: {
        threadId: payload.threadId
  ...
  const parsed = ThreadEventsBatchResponseSchema.safeParse(result.data);
  if (!parsed.success) {
    return { accepted: 0 };
  }
...
export function parseThreadStreamEventPayload(payload: unknown): ChatEvent | null {
  const parsed = ThreadEventSchema.safeParse(payload);
