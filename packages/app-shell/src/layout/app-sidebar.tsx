...
export function AppSidebar({ auth, buildSettingsHref }: AppSidebarProps) {
  ...
  const utilityItems: UtilityNavItem[] = [
    ...
    {
      label: "Automations",
      to: defaultWorkspaceSlug
        ? `/w/${encodeURIComponent(defaultWorkspaceSlug)}/automations`
        : "/workspaces",
      icon: Clock3,
      active: /^\/w\/[^/]+\/automations(?:\/|$)/u.test(location.pathname)
    },
    {
      label: "Skills",
      to: defaultWorkspaceSlug
        ? `/w/${encodeURIComponent(defaultWorkspaceSlug)}/skills`
        : "/workspaces",
      icon: Boxes,
      active: /^\/w\/[^/]+\/skills(?:\/|$)/u.test(location.pathname)
    }
  ];
