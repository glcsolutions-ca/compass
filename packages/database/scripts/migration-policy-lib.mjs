...
import { CHECKSUMS_PATH, MIGRATIONS_DIR } from "./paths.mjs";
...
export async function validateMigrationDirectory({ migrationsDir = MIGRATIONS_DIR } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (!entry.name.endsWith(".mjs")) {
      failures.push(`Non-.mjs file found in packages/database/migrations: ${entry.name}`);
      continue;
    }
  ...
  if (migrationFiles.length === 0) {
    failures.push("No migration files found in packages/database/migrations.");
  }
...
export async function validateMigrationPolicy({
  ...
  if (manifest) {
    ...
    for (const staleFile of [...expectedFiles].sort()) {
      failures.push(
        `Checksums manifest contains stale entry '${staleFile}' that is not present in packages/database/migrations.`
      );
    }
  }
