...
function resolveKeptRevisionNames(showDocument, labels) {
  const revisionNames = labels.map((label) => findLabelTraffic(showDocument, label)?.revisionName || '');

  if (revisionNames.some((revisionName) => !revisionName)) {
    throw new Error(`Container app is missing one or more required blue/green labels (${labels.join(', ')})`);
  }
