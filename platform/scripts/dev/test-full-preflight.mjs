...
export async function resolveDatabaseUrlForFullTest({
  ...
  const postgresEnvPath = path.resolve(rootDir, "packages/database/postgres/.env");
  ...
  return {
    ...
    source: normalizeEnvValue(env.POSTGRES_PORT)
      ...
      : normalizeEnvValue(layer.envLocalValues.POSTGRES_PORT)
        ? "packages/database/postgres/.env.local POSTGRES_PORT"
        : "packages/database/postgres/.env POSTGRES_PORT"
  };
}
