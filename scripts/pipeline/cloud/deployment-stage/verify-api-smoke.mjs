...
async function main() {
  ...
  try {
    ...
    const authStartHasLocation = authStartLocation.length > 0;
    let authStartAbsoluteUrl = null;
    try {
      authStartAbsoluteUrl = authStartLocation ? new URL(authStartLocation) : null;
    } catch {
      authStartAbsoluteUrl = null;
    }
    const authStartRedirectStatus = authStart.status === 302 || authStart.status === 303;
    const authStartRedirectShape =
      authStartHasLocation &&
      (authStartLocation.startsWith("/") ||
        authStartLocation.startsWith("http://") ||
        authStartLocation.startsWith("https://"));
    assertions.push({
    ...
    assertions.push({
      id: "auth-start-location-present",
      pass: authStartHasLocation,
      details: authStartLocation || "missing location header"
    });
    assertions.push({
      id: "auth-start-redirect-shape",
      pass: authStartRedirectShape,
      details:
