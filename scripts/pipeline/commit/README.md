...
| -------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------- | --------------------------------------------- |
| `resolve-scope.mjs`        | `.github/workflows/commit-stage.yml` | Resolve SHAs, classify scope (`runtime/infra/identity/docsOnly`), compute `changeClass`. | `.artifacts/commit-stage/<sha>/scope.json`    |
