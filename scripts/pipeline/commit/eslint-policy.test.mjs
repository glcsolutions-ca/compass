...
describe("policy-driven eslint test hygiene", () => {
  it("flags Math.random usage in commit-stage tests", async () => {
  ...
  it("flags direct pg imports in commit-stage tests", async () => {
    const messages = await lintMessages("import { Client } from 'pg';\nvoid Client;");
    expect(
      messages.some((message) => message.includes("Commit-stage tests must not import pg directly"))
    ).toBe(true);
  });

  it("flags focused tests in commit-stage tests", async () => {
