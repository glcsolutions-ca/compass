...
export async function verifyFromManifest({ environment, manifestPath }) {
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Manifest validation failed for verify command:\n${details}`);
  }

  const manifest = await readJsonFile(normalizedManifestPath);

  console.info(
    `Verification contract check passed for '${environment}' using candidate '${manifest.candidateId}'.`
  );
