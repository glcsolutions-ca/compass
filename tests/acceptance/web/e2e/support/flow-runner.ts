...
async function runForbiddenCreateThreadScenario({
  ...
  const threadCreateBlocker = async (route: Route) => {
    ...
    if (request.method() === "POST" && requestUrl.pathname === "/v1/threads") {
  ...
  await forbiddenPage.route("**/v1/threads*", threadCreateBlocker);
  ...
  try {
    ...
    flowAssertions.push({
      ...
      description: `[${flowId}] Forbidden create-thread branch intercepts POST /v1/threads`,
    ...
    await forbiddenPage.unroute("**/v1/threads*", threadCreateBlocker);
